...
public class Peripheral extends Activity implements ServiceFragment.ServiceFragmentDelegate {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        SharedPreferences pref = getSharedPreferences("SKRIPSI", MODE_PRIVATE);
        String valueUsername = pref.getString("USERNAME", "MAKANAN");
        Boolean marked = false;

        Bundle bd = getIntent().getExtras();
        if (bd!=null){
            marked = (Boolean) bd.get("MARKED");
        }

        // If we are not being restored from a previous state then create and add the fragment.
        if (savedInstanceState == null) {
            mCurrentServiceFragment = new ChessPieceFragment(valueUsername, marked);
            getFragmentManager()
                    .beginTransaction()
        ...
        Button viewPieces = (Button) findViewById(R.id.button_viewPieces);
        viewPieces.setOnTouchListener(new View.OnTouchListener() {
            @Override
    ...
    protected void onStart() {
        super.onStart();
        resetStatusViews();
        // If the user disabled Bluetooth when the app was in the background, openGattServer() will return null.
        mGattServer = mBluetoothManager.openGattServer(this, mGattServerCallback);
        if (mGattServer == null) {
            ensureBleFeaturesAvailable();
            return;
        }
        // Add a service for a total of three services (Generic Attribute and Generic Access are present by default).
